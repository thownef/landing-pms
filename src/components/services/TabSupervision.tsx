...
export function TabSupervision({ content }: TabSupervisionProps) {
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col lg:flex-row items-start gap-8 border-b border-gray-200 pb-8">
        ...
        {content.anh1?.url && (
          <div className="w-full lg:w-2/5 h-72 rounded-2xl overflow-hidden border border-gray-200 shadow-md relative group">
            ...
            <Image
              ...
              fill
              className="object-cover transform duration-500 group-hover:scale-105"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="flex items-center space-x-2">
            ...
            <h4 className="text-sm font-bold uppercase tracking-widest text-emerald-950">{content.tieuDe3}</h4>
          </div>
                ...
                {content.projects.map((row, index) => (
                  <tr key={index} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-3 py-3 text-center whitespace-nowrap">
                      <span className={`inline-flex px-2 py-0.5 rounded border font-semibold ${serviceRoleStyle(row.color)}`}>{row.role}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
